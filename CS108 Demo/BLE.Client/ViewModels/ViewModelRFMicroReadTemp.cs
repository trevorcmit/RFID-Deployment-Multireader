...
namespace BLE.Client.ViewModels {
    public class ViewModelRFMicroReadTemp : BaseViewModel {
        ...
        public ViewModelRFMicroReadTemp(IAdapter adapter, IUserDialogs userDialogs) : base(adapter) {
            ...
            _dataBank[0] = CSLibrary.Constants.MemoryBank.BANK2; _dataOffset[0] = 0; _dataCount[0] = 2; // RWTagID
            _dataBank[1] = CSLibrary.Constants.MemoryBank.BANK3; _dataOffset[1] = 8; _dataCount[1] = 4; // calibration
        ...
        public override void Resume() {
            base.Resume();
            BleMvxApplication._reader1.rfid.OnAccessCompleted += new EventHandler<CSLibrary.Events.OnAccessCompletedEventArgs>(TagCompletedEvent1);
            BleMvxApplication._reader2.rfid.OnAccessCompleted += new EventHandler<CSLibrary.Events.OnAccessCompletedEventArgs>(TagCompletedEvent2);
        }
        ...
        void TagCompletedEvent1(object sender, CSLibrary.Events.OnAccessCompletedEventArgs e) {
            if (e.access == CSLibrary.Constants.TagAccess.READ) {
                if (e.bank == CSLibrary.Constants.Bank.SPECIFIC) {
                    ...
                    switch (_readProcedure) {
                        case 0:
                            if (e.success) {
                                entryRWTagID = BleMvxApplication._reader1.rfid.Options.TagRead.pData.ToString();
                        ...
                        case 1:
                            if (e.success) {
                                entryCalibration = BleMvxApplication._reader1.rfid.Options.TagRead.pData.ToString();
                                RaisePropertyChanged(() => entryCalibration);
                                labelCalibrationStatus = "Ok";
                                RaisePropertyChanged(() => labelCalibrationStatus);
                                {
                                    UInt16 [] cal = BleMvxApplication._reader1.rfid.Options.TagRead.pData.ToUshorts();
                        ...
                        case 2:
                            if (e.success)
                            {
                                entrySensorCode = BleMvxApplication._reader1.rfid.Options.TagRead.pData.ToString();
                        ...
                        case 3:
                            if (e.success) {
                                entryRssiCode = BleMvxApplication._reader1.rfid.Options.TagRead.pData.ToString();
                                RaisePropertyChanged(() => entryRssiCode);
                                labelRssiCodeStatus = "Ok";
                                RaisePropertyChanged(() => labelRssiCodeStatus);

                                {
                                    UInt16 rssi = BleMvxApplication._reader1.rfid.Options.TagRead.pData.ToUshorts()[0];
                        ...
                        case 4:
                            if (e.success)
                            {
                                UInt16 [] var = BleMvxApplication._reader1.rfid.Options.TagRead.pData.ToUshorts();

                                entryTemperatureCode = BleMvxApplication._reader1.rfid.Options.TagRead.pData.ToString();
        ...
        void TagCompletedEvent2(object sender, CSLibrary.Events.OnAccessCompletedEventArgs e) {
            if (e.access == CSLibrary.Constants.TagAccess.READ) {
                if (e.bank == CSLibrary.Constants.Bank.SPECIFIC) {
                    if (!e.success) {
                        if (--readRetryCnt > 0) {
                            BleMvxApplication._reader2.rfid.StartOperation(CSLibrary.Constants.Operation.TAG_READ);
                            return;
                        }
                    }
                    ...
                    switch (_readProcedure) {
                        case 0:
                            if (e.success) {
                                entryRWTagID = BleMvxApplication._reader2.rfid.Options.TagRead.pData.ToString();
                                RaisePropertyChanged(() => entryRWTagID);
                                labelRWTagIDStatus = "Ok";
                                RaisePropertyChanged(() => labelRWTagIDStatus);
                            }
                        ...
                        case 1:
                            if (e.success) {
                                entryCalibration = BleMvxApplication._reader2.rfid.Options.TagRead.pData.ToString();
                                RaisePropertyChanged(() => entryCalibration);
                                labelCalibrationStatus = "Ok";
                                RaisePropertyChanged(() => labelCalibrationStatus);
                                {
                                    UInt16 [] cal = BleMvxApplication._reader2.rfid.Options.TagRead.pData.ToUshorts();
                                    _calibrationCode = (UInt64)(((UInt64)cal[0] << 48) | ((UInt64)cal[1] << 32) | ((UInt64)cal[2] << 16) | ((UInt64)cal[3]));
                                }
                            }
                            else {
                        ...
                        case 2:
                            if (e.success) {
                                entrySensorCode = BleMvxApplication._reader2.rfid.Options.TagRead.pData.ToString();
                                RaisePropertyChanged(() => entrySensorCode);
                                labelSensorCodeStatus = "Ok";
                                RaisePropertyChanged(() => labelSensorCodeStatus);
                            }
                        ...
                        case 3:
                            if (e.success) {
                                entryRssiCode = BleMvxApplication._reader2.rfid.Options.TagRead.pData.ToString();
                                RaisePropertyChanged(() => entryRssiCode);
                                labelRssiCodeStatus = "Ok";
                                RaisePropertyChanged(() => labelRssiCodeStatus);

                                {
                                    UInt16 rssi = BleMvxApplication._reader2.rfid.Options.TagRead.pData.ToUshorts()[0];
                                    labelOCRSSIText = rssi.ToString("D");
                        ...
                        case 4:
                            if (e.success) {
                                UInt16 [] var = BleMvxApplication._reader2.rfid.Options.TagRead.pData.ToUshorts();

                                entryTemperatureCode = BleMvxApplication._reader2.rfid.Options.TagRead.pData.ToString();
                                RaisePropertyChanged(() => entryTemperatureCode);
                                labelTemperatureCodeStatus = "Ok";
                                RaisePropertyChanged(() => labelTemperatureCodeStatus);
                                RaisePropertyChanged(() => labelTemperatureText);
        ...
        void ReadNextData (int n) {
            if (n==1) {
                ...
                _readProcedure++;
                if (SetParameters1()) BleMvxApplication._reader1.rfid.StartOperation(CSLibrary.Constants.Operation.TAG_READ);
            }
            else if (n==2) {
                if (_readProcedure >= 5) {
                ...
                _readProcedure++;
                if (SetParameters2()) BleMvxApplication._reader2.rfid.StartOperation(CSLibrary.Constants.Operation.TAG_READ);
            }
        }
        ...
        bool SetParameters1 () {
            switch (_readProcedure)
            {
                case 0: //
                    ...
                    {
                        _readProcedure++;
                        return SetParameters1();
                    }
                    BleMvxApplication._reader1.rfid.Options.TagRead.bank =  (CSLibrary.Constants.MemoryBank)_dataBank[0];
                    BleMvxApplication._reader1.rfid.Options.TagRead.offset = _dataOffset[0];
                    BleMvxApplication._reader1.rfid.Options.TagRead.count = _dataCount[0];
                    BleMvxApplication._reader1.rfid.StartOperation(CSLibrary.Constants.Operation.TAG_SELECTED);
                    BleMvxApplication._reader1.rfid.SetSelectCriteria(1, null);
                    break;
                case 1:
                    ...
                    BleMvxApplication._reader1.rfid.Options.TagRead.bank = _dataBank[1];
                    BleMvxApplication._reader1.rfid.Options.TagRead.offset = _dataOffset[1];
                    BleMvxApplication._reader1.rfid.Options.TagRead.count = _dataCount[1];
                    BleMvxApplication._reader1.rfid.StartOperation(CSLibrary.Constants.Operation.TAG_SELECTED);
                    BleMvxApplication._reader1.rfid.SetSelectCriteria(1, null);
                    break;
                case 2:
                    ...
                    BleMvxApplication._reader1.rfid.Options.TagRead.bank = _dataBank[_tagMode + 1];
                    BleMvxApplication._reader1.rfid.Options.TagRead.offset = _dataOffset[_tagMode + 1];
                    BleMvxApplication._reader1.rfid.Options.TagRead.count = _dataCount[_tagMode + 1];
                    {
                        BleMvxApplication._reader1.rfid.StartOperation(CSLibrary.Constants.Operation.TAG_SELECTED);
                        BleMvxApplication._reader1.rfid.SetTagGroup(CSLibrary.Constants.Selected.ASSERTED, CSLibrary.Constants.Session.S1, CSLibrary.Constants.SessionTarget.A); // better for read rangage
                ...
                case 3:
                    ...
                    if (_tagMode == 1)
                    {
                        BleMvxApplication._reader1.rfid.Options.TagRead.bank = _dataBank[5];
                        BleMvxApplication._reader1.rfid.Options.TagRead.offset = _dataOffset[5];
                        BleMvxApplication._reader1.rfid.Options.TagRead.count = _dataCount[5];
                    }
                    else
                    {
                        BleMvxApplication._reader1.rfid.Options.TagRead.bank = _dataBank[6];
                        BleMvxApplication._reader1.rfid.Options.TagRead.offset = _dataOffset[6];
                        BleMvxApplication._reader1.rfid.Options.TagRead.count = _dataCount[6];
                    }
                    {
                        BleMvxApplication._reader1.rfid.StartOperation(CSLibrary.Constants.Operation.TAG_SELECTED);
                        BleMvxApplication._reader1.rfid.SetTagGroup(CSLibrary.Constants.Selected.ASSERTED, CSLibrary.Constants.Session.S1, CSLibrary.Constants.SessionTarget.A); // better for read rangage
                ...
                case 4:
                    ...
                    BleMvxApplication._reader1.rfid.Options.TagRead.bank = _dataBank[7];
                    BleMvxApplication._reader1.rfid.Options.TagRead.offset = _dataOffset[7];
                    BleMvxApplication._reader1.rfid.Options.TagRead.count = _dataCount[7];
        ...
        bool SetParameters2 () {
            switch (_readProcedure) {
                case 0: //
                    if (!switchRWTagIDIsToggled) {
                        _readProcedure++;
                        return SetParameters2();
                    }
                    BleMvxApplication._reader2.rfid.Options.TagRead.bank =  (CSLibrary.Constants.MemoryBank)_dataBank[0];
                    BleMvxApplication._reader2.rfid.Options.TagRead.offset = _dataOffset[0];
                    BleMvxApplication._reader2.rfid.Options.TagRead.count = _dataCount[0];
                    BleMvxApplication._reader2.rfid.StartOperation(CSLibrary.Constants.Operation.TAG_SELECTED);
                    BleMvxApplication._reader2.rfid.SetSelectCriteria(1, null);
                    break;
                case 1:
                    if (!switchCalibrationIsToggled) {
                        _readProcedure++;
                    ...
                    BleMvxApplication._reader2.rfid.Options.TagRead.bank = _dataBank[1];
                    BleMvxApplication._reader2.rfid.Options.TagRead.offset = _dataOffset[1];
                    BleMvxApplication._reader2.rfid.Options.TagRead.count = _dataCount[1];
                    BleMvxApplication._reader2.rfid.StartOperation(CSLibrary.Constants.Operation.TAG_SELECTED);
                    BleMvxApplication._reader2.rfid.SetSelectCriteria(1, null);
                    break;
                case 2:
                    ...
                    BleMvxApplication._reader2.rfid.Options.TagRead.bank = _dataBank[_tagMode + 1];
                    BleMvxApplication._reader2.rfid.Options.TagRead.offset = _dataOffset[_tagMode + 1];
                    BleMvxApplication._reader2.rfid.Options.TagRead.count = _dataCount[_tagMode + 1];
                    {
                        BleMvxApplication._reader2.rfid.StartOperation(CSLibrary.Constants.Operation.TAG_SELECTED);
                        BleMvxApplication._reader2.rfid.SetTagGroup(CSLibrary.Constants.Selected.ASSERTED, CSLibrary.Constants.Session.S1, CSLibrary.Constants.SessionTarget.A);
                ...
                case 3:
                    ...
                    if (_tagMode == 1) {
                        BleMvxApplication._reader2.rfid.Options.TagRead.bank   = _dataBank[5];
                        BleMvxApplication._reader2.rfid.Options.TagRead.offset = _dataOffset[5];
                        BleMvxApplication._reader2.rfid.Options.TagRead.count  = _dataCount[5];
                    }
                    else {
                        BleMvxApplication._reader2.rfid.Options.TagRead.bank = _dataBank[6];
                        BleMvxApplication._reader2.rfid.Options.TagRead.offset = _dataOffset[6];
                        BleMvxApplication._reader2.rfid.Options.TagRead.count = _dataCount[6];
                    }
                    {
                        BleMvxApplication._reader2.rfid.StartOperation(CSLibrary.Constants.Operation.TAG_SELECTED);
                        BleMvxApplication._reader2.rfid.SetTagGroup(CSLibrary.Constants.Selected.ASSERTED, CSLibrary.Constants.Session.S1, CSLibrary.Constants.SessionTarget.A);
                ...
                case 4:
                    ...
                    BleMvxApplication._reader2.rfid.Options.TagRead.bank   = _dataBank[7];
                    BleMvxApplication._reader2.rfid.Options.TagRead.offset = _dataOffset[7];
                    BleMvxApplication._reader2.rfid.Options.TagRead.count  = _dataCount[7];
        ...
        void OnReadButtonButtonClick() {
            ...
            BleMvxApplication._reader1.rfid.Options.TagRead.accessPassword = Convert.ToUInt32(entrySelectedPWD, 16);
            BleMvxApplication._reader2.rfid.Options.TagRead.accessPassword = Convert.ToUInt32(entrySelectedPWD, 16);

            if (BleMvxApplication._reader1.rfid.State != CSLibrary.Constants.RFState.IDLE) {return;}
            if (BleMvxApplication._reader2.rfid.State != CSLibrary.Constants.RFState.IDLE) {return;}
            ...
            BleMvxApplication._reader2.rfid.Options.TagSelected.epcMask = new CSLibrary.Structures.S_MASK(/*m_record.pc.ToString() + */entrySelectedEPC);

            BleMvxApplication._reader1.rfid.Options.TagSelected.flags = CSLibrary.Constants.SelectMaskFlags.ENABLE_TOGGLE;
            BleMvxApplication._reader2.rfid.Options.TagSelected.flags = CSLibrary.Constants.SelectMaskFlags.ENABLE_TOGGLE;

            BleMvxApplication._reader1.rfid.Options.TagSelected.epcMaskOffset = 0;
            BleMvxApplication._reader2.rfid.Options.TagSelected.epcMaskOffset = 0;
            ...
            BleMvxApplication._reader2.rfid.Options.TagSelected.epcMaskLength = (uint)BleMvxApplication._reader2.rfid.Options.TagSelected.epcMask.Length * 8;
